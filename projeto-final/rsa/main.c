...
int main(int argc, char *argv[]) {
    ...
    setlocale(LC_ALL, "Portuguese");    // suportar UTF-8
    ...
    // Array de char para armazenar a mensagem de entrada
    char *mensagem;
    ...
    // Array para armazenar os números correspondentes aos caracteres
    long long *numeros;
    ...
    //printf("Entrada n = %ld\nTempo de criptografia com %d threads: %f segundos\n", tamanhoArq, num_threads, elapsed_c);
    ...
    // Arquivo de saída (mensagem descriptografada)
    arq = fopen(argv[2], "w");
    if (arq == NULL) {
        printf("Erro ao abrir arquivo");
        exit(-1);
